...
func AddGlobalFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().String(ConfigFlag, "", "Config file (default $HOME/.config/nix-foundry/config.yaml)")
	cmd.PersistentFlags().Bool("debug", false, "Enable debug logging")
}

// AddForceFlag adds --force to a command
func AddForceFlag(cmd *cobra.Command) {
	cmd.Flags().BoolP(ForceFlag, "f", false, "Force operation despite conflicts")
}

// AddYesFlag adds --yes for automatic confirmation
func AddYesFlag(cmd *cobra.Command) {
	cmd.Flags().BoolP(YesFlag, "y", false, "Automatic yes to prompts")
}
