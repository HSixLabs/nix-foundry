...
rec {
  ...
  platformSettings = system: {
    homePrefix = if isDarwin system then "/Users"
                else if isWindows system then "C:/Users"
                else "/home";
                
    pathSeparator = if isWindows system then "\\" else "/";
    
    defaultShell = if isWindows system then "pwsh"
                  else "zsh";
                  
    configDir = if isWindows system then "$APPDATA/nix-configs"
                else "$HOME/.config/nix-configs";
  };
} 